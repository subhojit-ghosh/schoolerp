...
export function DataTable<TData, TValue>({
  ...
  return (
    <div className="w-full space-y-2.5">
      {searchKey && (
        <div className="flex flex-col gap-2.5 rounded-[22px] border border-border/70 bg-[var(--shell-elevated)] p-2.5 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full max-w-xl">
            ...
            <Input
              ...
              className="h-10 rounded-2xl border-border/70 bg-background/90 pr-4 pl-10 text-sm shadow-none"
            />
          </div>
          {toolbarContent ? (
      ...
      <div className="overflow-hidden rounded-[22px] border border-border/70 bg-[var(--shell-elevated)] shadow-[0_18px_40px_-32px_rgba(15,45,53,0.45)]">
        <Table className="[&_td]:px-4 [&_td]:py-2.5 [&_th]:px-4 [&_th]:py-2.5">
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-border/70 bg-muted/55 hover:bg-muted/55">
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  key={row.id}
                  className="border-border/70 bg-transparent hover:bg-background/45"
                >
      ...
      <div className="flex items-center justify-between rounded-[20px] border border-border/70 bg-[var(--shell-elevated)] px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[72px] rounded-xl border-border/70 bg-background/90">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <Button
              variant="outline"
              size="icon"
              className="size-8 rounded-xl border-border/70 bg-background/90"
              onClick={() => {
...
export function SortableHeader({
  ...
  return (
    <Button
      variant="ghost"
      className="-ml-2 h-7 text-[0.8125rem]"
